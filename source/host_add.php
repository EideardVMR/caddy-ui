<div class="view">
    <div class="content small"">
        ...
        $host = new Host();
        if (isset($_POST['save'])) {
            $host->domain = $_POST['fulldomain'] ?? $_POST['subdomain'] . '.' . $_POST['domain'];
            ...
            if ($host->validate() === false) {
                ...
                if (HostDB::addHost($host)) {
                    echo '<div class="message success">Host erfolgreich hinzugefügt</div>';
        ...
        <form method="post" action="?s=host_add">

        <h3>Domain</h3>
        <?php if (DOMAINS === false): ?>
            <div class="field">
                <label for="fulldomain">Domain</label>
                <input id="fulldomain" name="fulldomain" type="text" value="<?= $host ? $host->domain : '' ?>" placeholder="z.B. test.example.de">
            </div>
        <?php else: ?>
            <div style="display:flex; justify-content: space-between; gap: 10px">
                <div class="field" style="width: 50%">
                    <label for="subdomain">Sub-Domain</label>
                    <input id="subdomain" name="subdomain" type="text" value="<?= $host ? $host->getSubDomain() : '' ?>" placeholder="z.B. test">
                </div>
                <div class="field" style="width: 50%">
